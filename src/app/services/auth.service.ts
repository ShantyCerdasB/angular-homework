...
export class AuthService {
  ...
  public isSuperAdmin(): boolean {
    return this.user.authorities ?  this.user?.authorities.some(authority => authority.authority == IRoleType.ROLE_SUPER_ADMIN_ROLE) : false;
  }
  ...
  public areActionsAvailable(routeAuthorities: string[]): boolean  {
    ...
    if (userAuthorities?.some(item => item.authority == IRoleType.ROLE_SUPER_ADMIN_ROLE || item.authority == IRoleType.ROLE_SUPER_ADMIN_ROLE)) {
      isAdmin = userAuthorities?.some(item => item.authority == IRoleType.ROLE_SUPER_ADMIN_ROLE || item.authority == IRoleType.ROLE_SUPER_ADMIN_ROLE);
    }          
